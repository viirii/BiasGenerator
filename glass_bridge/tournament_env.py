...
class GlassBridgeTournamentEnv:
        ...
        return [
            {
                ...
                "private_offer_pairs": sorted(
                    {self._pair_key(offer["proposer"], offer["recipient"]) for offer in self.pending_offers}
                ),
            }
        ]
    ...
    def _apply_response_actions(self, action_dict: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for agent_name in self.current_order:
            ...
            accepted_ids = {int(offer_id) for offer_id in action.get("accept_offer_ids", [])}
    ...
    def _complete_round(self) -> tuple[list[dict[str, Any]], bool]:
        ...
        events: list[dict[str, Any]] = [{"type": "round_end", **round_summary}]
    ...
    def _result(self, rewards: dict[str, float], done: bool, events: list[dict[str, Any]]) -> dict[str, Any]:
        return {
    ...
    def _normalize_offer_steps(self, owner: str, step_values: list[Any], known_steps: dict[int, int]) -> list[int]:
        normalized: list[int] = []
